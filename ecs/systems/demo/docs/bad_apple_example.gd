...
##
## IMPORTANT: This example shows the CORRECT usage pattern:
## 1. Use call_deferred() for setup in _ready()
## 2. Create VideoStreamPlayer programmatically and add as child
## 3. await get_tree().process_frame after video player creation
## 4. Call FlecsServer.progress_world(world_rid, delta) in _process()
...
func _ready():
	...
	# call_deferred("setup_high_performance_example")
	# call_deferred("setup_low_end_hardware_example")
	# call_deferred("setup_benchmark_example")
...
func setup_basic_example():
	...
	# Create video player programmatically (not from scene)
	video_player = VideoStreamPlayer.new()
	add_child(video_player)
	video_player.stream = load("res://demos/bad apple/bad_apple.ogv")
	video_player.autoplay = true

	# CRITICAL: Wait one frame to ensure video player has processed NOTIFICATION_ENTER_TREE
	await get_tree().process_frame

	# Create multimesh entity
...
func setup_low_end_hardware_example():
	...
	bad_apple_system.set_max_threads(4)              # Limit threads
	bad_apple_system.set_mode(0)
...
func setup_benchmark_example():
	...
	# Calculate speedup
	var speedup = single_avg / multi_avg if multi_avg > 0 else 0
	print("=== RESULTS ===")
...
func demonstrate_mode_switching():
	...
	bad_apple_system.set_mode(0)  # Black & white

	await get_tree().create_timer(5.0).timeout

	print("Switching to INVERTED mode")
	bad_apple_system.set_mode(1)  # Inverted black & white

	await get_tree().create_timer(5.0).timeout

	print("Switching to RANDOM mode")
	bad_apple_system.set_mode(2)  # Random colors (uses fast hash)
...
func adaptive_threading_example():
	...
	while true:
		...
		if current_avg > target_frame_time_us * 0.8:
			if bad_apple_system.get_max_threads() < 16:
				bad_apple_system.set_max_threads(bad_apple_system.get_max_threads() + 2)
				print("Increasing threads to: %d" % bad_apple_system.get_max_threads())

		# If we have headroom, reduce threads to save power
		elif current_avg < target_frame_time_us * 0.3:
			if bad_apple_system.get_max_threads() > 2:
				bad_apple_system.set_max_threads(bad_apple_system.get_max_threads() - 2)
				print("Reducing threads to: %d" % bad_apple_system.get_max_threads())
...
## CRITICAL: Display real-time performance stats and progress the ECS world
func _process(delta):
