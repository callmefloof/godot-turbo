...
public:
	...
	static Ref<ScreenAABB> compute_2d_aabb(const Vector2& v0, const Vector2& v1, const Vector2& v2);
	void clear_bins();
	void reset();
	void reset(const int p_screen_width, const int p_screen_height);
	void bin_triangles(const Vector<Ref<ScreenTriangle>> &tris);
	...
	static void rasterize_triangle_to_tile(const Ref<ScreenTriangle>& tri, TileBuffer& tile, const Vector2i & tile_origin);

	void rasterize_all_bins();

	static bool test_visibility(const Ref<ScreenAABB> &box, const float *occlusion_buffer, const int buffer_width, const int buffer_height);
	bool is_visible(const Ref<ScreenAABB> &aabb) const;
	std::vector<bool> test_all_visibility_concurrent(const std::vector<Ref<ScreenAABB>> &boxes, unsigned int thread_count) const;
